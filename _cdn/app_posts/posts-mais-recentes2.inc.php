<?php
$readPostRecentes = new Read;
$readPostRecentes->FullRead("SELECT post_title, post_name, post_content, post_date, post_cover, post_category FROM " . POSTS . " WHERE post_id != :id AND post_status = 1 ORDER BY post_date DESC LIMIT 3", "id={$readPost->getResult()[0]['post_id']}");

if ($readPostRecentes->getResult()):
    ?>
    ...
    <section class="container post_sidebar">
        ...
        <?php
        $j = 0;
        foreach ($readPostRecentes->getResult() as $post):
            extract($post);
            ?>
