...
class adminUser {
    ...
    public function ExeCreate(array $Data) {
        ...
        elseif (!Check::Email($this->Data['user_email'])):
            $this->Error = ["Formato de E-mail Inválido", "error"];
            $this->Result = false;
...
//            var_dump($readUser->getResult()[0]['user_password'], substr(md5($this->Data['user_password']), 0, 16), $this->Data['user_password']);
    ...
    private function verificarEmail() {
        ...
        if ($read->getResult()):
            $this->Error = ["Este email já existe! Favor colocar outro.", "alert"];
            $this->Result = false;
        else:
            $this->Result = true;
        endif;
    ...
    private function Create() {
        ...
        if (!$create->getResult()):
            $this->Error = ['Não foi possível cadastrar User', "error"];
            $this->Result = false;
        else:
            $this->Error = ['User cadastrado com Sucesso!', "success"];
            $this->Result = true;
        endif;
    }
    ...
    private function Update() {
        ...
        if (!$update->getResult()):
            $this->Error = ["Erro ao Atualizar User", "error"];
            $this->Result = false;
        else:
            $this->Error = ["User Atualizado com Sucesso!", "success"];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if (!$delete->getResult()):
            $this->Error = ["Erro ao Excluir User", "error"];
            $this->Result = false;
        else:
            $this->Error = ["User Excluído com Sucesso!", "success"];
            $this->Result = true;
        endif;
    }
