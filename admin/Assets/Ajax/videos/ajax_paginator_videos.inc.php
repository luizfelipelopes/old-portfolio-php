...
/**
 * ajax_paginator_videos.inc.php - <b>PAGINATOR POSTS</b>
 * Arquivo de inclusão do ajax.php para armazenar o script de Paginação de Posts
 */
if (!empty($Post)):
    ...
    $Pager->ExePager($Post['paginator'], 6);
    $readVideos = new Read;
    ...
    if ($readVideos->getResult()):
        ...
        foreach ($readVideos->getResult() as $video):
            ...
            if ($video_status == 1):
                ...
                $jSon['videos'] .= '<a id="' . $video_id . '" attr-status="' . $video_status . '" attr-action="change_status_post" title="Publicado" href="#" class="icon-check btn_status btn_status_published btn btn-small btn-green radius js_status">Publicado</a>';
            else:
                $jSon['videos'] .= '<a id="' . $video_id . '" attr-status="' . $video_status . '" attr-action="change_status_post" title="Rascunho" href="#" class="icon-alert-triangle btn_status btn_status_draft btn btn-small btn-orange radius js_status">Rascunho</a>';
