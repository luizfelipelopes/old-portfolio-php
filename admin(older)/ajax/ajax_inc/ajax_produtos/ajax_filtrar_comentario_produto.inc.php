...
/**
 * ajax_filtrar_comentario_produto.inc.php - <b>FILTRAR COMENTARIO PRODUTO</b>
 * Arquivo de inclusão do ajax.php para armazenar o script de Filtro de Comentário de Produtos
 */
$readComentarios = new Read;

if (empty($Post['produto'])):
    $readComentarios->ExeRead(COMENTARIOS, "WHERE comentario_resposta IS NULL ORDER BY comentario_date DESC");
else:
    $readComentarios->ExeRead(COMENTARIOS, "WHERE comentario_resposta IS NULL AND comentario_produto = :produto ORDER BY comentario_date DESC", "produto={$Post['produto']}");
endif;
