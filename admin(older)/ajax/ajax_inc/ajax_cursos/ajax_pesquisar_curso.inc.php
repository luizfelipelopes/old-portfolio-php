...
/**
 * ajax_pesquisar_curso.inc.php - <b>PESQUISAR CURSO</b>
 * Arquivo de inclusão do ajax.php para armazenar o script de Pesquisa de Cursos em REAL_TIME
 */
$read = new Read;
...
$read->ExeRead(CURSOS, "WHERE (curso_title LIKE '%' :like '%') ORDER BY curso_date DESC LIMIT :limit OFFSET :offset", "like={$Post['s']}&limit={$Pager->getLimit()}&offset={$Pager->getOffset()}");
if (!$read->getResult()):
    $Pager->ReturnPage();
