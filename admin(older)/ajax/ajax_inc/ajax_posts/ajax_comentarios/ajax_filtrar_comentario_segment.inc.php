...
$readComentarios->FullRead("SELECT COUNT(comentario_id) AS TOTAL, comentario_status, " . $tabela_join_title . " , " . $tabela_grupo_type . $tabela_join_cover . " FROM " . COMENTARIOS . " a LEFT JOIN " . $tabela_join . " b ON a." . $tabela_grupo_type . " = b." . $tabela_join_id . " WHERE " . $sql_segment . ' ' . $tabela_grupo_type . " IS NOT NULL " . (!empty($Post['filtro']) ? "AND " . $tabela_join_id . " = :filtro" : '') . " GROUP BY " . $tabela_grupo_type . " ORDER BY comentario_date DESC" . $tabela_coluna_type . " LIMIT :limit OFFSET :offset", (!empty($Post['filtro']) ? "filtro={$Post['filtro']}&" : '') . "limit={$Pager->getLimit()}&offset={$Pager->getOffset()}&segment={$Post['segment']}");
if (!$readComentarios->getResult()):
    ...
    $Pager->ExeFullPaginator("SELECT COUNT(comentario_id) AS TOTAL, comentario_status, " . $tabela_join_title . " , " . $tabela_grupo_type . $tabela_join_cover . " FROM " . COMENTARIOS . " a LEFT JOIN " . $tabela_join . " b ON a." . $tabela_grupo_type . " = b." . $tabela_join_id . " WHERE " . $sql_segment . ' ' . $tabela_grupo_type . " IS NOT NULL " . (!empty($Post['filtro']) ? "AND " . $tabela_join_id . " = :filtro" : '') . " GROUP BY " . $tabela_grupo_type . " ORDER BY comentario_date DESC" . $tabela_coluna_type, (!empty($Post['filtro']) ? "filtro={$Post['filtro']}&" : '') . "segment={$Post['segment']}");
