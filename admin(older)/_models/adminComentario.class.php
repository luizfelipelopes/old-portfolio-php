...
class adminComentario {
    ...
    public function ExeCreate(array $Data) {
        ...
        if (isset($this->Data['comentario_author']) && $this->Data['comentario_author'] == null || isset($this->Data['comentario_email']) && $this->Data['comentario_email'] == null || isset($this->Data['comentario_content']) && $this->Data['comentario_content'] == null):
            $this->Error = ["Por favor, preencha os campos Obrigatórios (Nome, email e mensagem)", WS_ERROR];
            $this->Result = false;
    ...
    private function Create() {
        ...
        if (!$create->getResult()):
            $this->Error = ['Não foi possível cadastrar Comentário', WS_ERROR];
            $this->Result = false;
        else:
            $this->Error = [(MODERADOR == '1' ? 'Parabéns! Sua Avaliação foi enviada a moderação! ;)' : 'Comentário Enviado com Sucesso! =D'), WS_ACCEPT];
    ...
    private function Update() {
        ...
        if (!$update->getResult()):
            $this->Error = ["Erro ao Atualizar Comentário", WS_ERROR];
            $this->Result = false;
        else:
            $this->Error = ["Comentário Atualizado com Sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
    ...
    private function Delete() {
        ...
        if (!$delete->getResult()):
            $this->Error = ["Erro ao Excluir Comentário", WS_ERROR];
            $this->Result = false;
        else:
            $this->Error = ["Comentário Excluído com Sucesso!", WS_ACCEPT];
            $this->Result = true;
        endif;
    }
